...
### 1. Set parameters --------
  ...
  roads_grip <- get_vector("data/raw/vector/GRIP_roads", match = COUNTRY, source = "grip") %>%
    st_transform(CRS)
  roads_osm <- get_osm("data/raw/vector/osm", match = COUNTRY) %>%  # Not loading full dataset; try converting to gpkg first
    st_transform(CRS)
  ...
  ## (i) Forest cover in start year (7 min for Colombia)
...
grid_pa_intersection_fc <- grid_pa_intersection %>%
  ...
  mutate(frac_protected = fc_protected / (fc_start * poly_area)) %>%
  select(-weighted_mean, -int_area) %>%
  st_drop_geometry()

## (vii) Calculate fractional polygon overlap with jurisdictional and ecoregion boundaries
...
# tic(); roads_rast_osm <- rasterize_lines(roads_osm, fc_start_agg); toc()

# roads_rast_all <- max(c(roads_rast_grip, roads_rast_osm))  ## FIX OSM DATA

# Calculate distance rasters
tic()
dist_to_river <- distance(rivers_rast)
tic()
dist_to_road <- distance(roads_rast_grip)
toc()

### 6. Extract annual forest loss per polygon in poly and buffer --------

grid_fc_loss <- grid_threshold %>%
  poly_extract(fc_loss) %>%
  rename_with(str_replace, .cols = starts_with("weighted_mean"), pattern = "weighted_mean.", replacement = "")

grid_buffer_fc_loss <- poly_extract(grid_buffer_threshold, fc_loss)
